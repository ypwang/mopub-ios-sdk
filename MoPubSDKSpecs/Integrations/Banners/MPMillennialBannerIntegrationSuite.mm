...
describe(@"MPMillennialBannerIntegrationSuite", ^{
    __block FakeMMBannerAdView *fakeAd;
    ...
    beforeEach(^{
        ...
        fakeAd = [[[FakeMMBannerAdView alloc] initWithFrame:CGRectMake(0,0,20,30)] autorelease];
        fakeProvider.fakeMMAdViewBanner = fakeAd;
    ...
    it(@"should ask the ad to load", ^{
        ...
        fakeAd.delegate.requestData[@"lat"] should equal(@"37.1");
        fakeAd.delegate.requestData[@"long"] should equal(@"21.2");
        fakeAd.rootViewController should equal(presentingController);
        fakeAd.frame should equal(CGRectMake(0, 0, 300, 250));
        fakeAd.type should equal(MMBannerAdRectangle);
        fakeAd.hasRefreshed should equal(YES);
    });
